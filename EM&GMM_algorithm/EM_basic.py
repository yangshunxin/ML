...
def multivariate_normal(mean, cov, size=None, check_valid=None, tol=None)
    ...
    size: 指定生成的正太分布矩阵的维度，
'''
data1 = np.random.multivariate_normal(mean1, cov1, N)  # shape: (500, 3) 三个特征/属性
# print(data1.shape)

## 产生一个分布分布不均衡的数据集， 100条
mean2 = (10, 12, 1)
cov2 = np.array(((1, 1, 3), (1, 2, 1), (0, 0, 1)))
data2 = np.random.multivariate_normal(mean2, cov2, M) # (250, 3)
# print(data2.shape)

## 合并data1和data2这两个数据集
data = np.vstack((data1, data2)) #  数据堆叠起来 (750, 3)
# print(data.shape)

## 产生数据对应的y值
y1 = np.array([True]*N + [False]*M)  # (750,)
y2 = ~y1
# print(y1)
print(y1.shape)
...
# 预测分类 norm1和norm2 为预测的模型，，
norm1 = multivariate_normal(mu1, sigma1) # 生成多元正太分布矩阵
norm2 = multivariate_normal(mu2, sigma2)
tau1 = norm1.pdf(data) # (750,)---概率值
tau2 = norm2.pdf(data)
# print(tau1)

## 计算均值的距离，然后根据距离得到分类情况
dist = pairwise_distances_argmin([mean1, mean2], [mu1, mu2], metric='euclidean') # [0 1]
print ("距离:", dist)
if dist[0] == 0:
    c1 = tau1 > tau2  # 以tau1为主，
else:
    c1 = tau1 < tau2
c2 = ~c1
